...
## Stage 5.3 — Invoice module (item 29), backend

Fourteen new whitelisted endpoints. **Every one of them calls `require_module()`
first**, which raises `frappe.PermissionError` while `invoices_enabled` is off —
hiding a page is not a flag, because the endpoint is still reachable by URL. The
public `view` route is the one exception in form only: while the flag is off it
answers exactly like a dead token, so an anonymous caller learns nothing about
the site's configuration.

Row-level scope is derived ONCE, in one place: an invoice has no scope of its
own, it belongs to the deal it bills. `CRM Invoice` registers
`get_invoice_permission_query_conditions` (list queries) and
`has_invoice_permission` (single documents) in `crm/hooks.py`, and both wrap the
deal's org-hierarchy conditions from `crm/permissions/org_hierarchy.py`. No
endpoint below re-derives scope, and no endpoint takes a filter from the request.

| Endpoint | Roles | Scope derivation | Record checks | Tested in |
| --- | --- | --- | --- | --- |
| `crm.api.invoices.convert_deal` (POST) | Sales User, Sales Manager, System Manager | `get_deal_doc(deal, "write")` — `frappe.has_permission("CRM Deal", "write", name)`, so the org-hierarchy `has_deal_permission` hook decides. The deal name is the only scope input; the customer block is read off the deal, never off the request | `CRM Deal` write; `CRM Invoice` create (`frappe.has_permission`) then `doc.insert()`, which applies the doctype's permissions again; `CRM Organization` / `Address` read only through the deal's own link | `test_invoices.py::TestAC9Permissions::test_a_sales_user_outside_the_deal_cannot_read_the_invoice`, `::TestAC10Flag::test_every_endpoint_refuses_while_the_flag_is_off` |
| `crm.api.invoices.get_invoice` (GET) | Sales User, Sales Manager, System Manager | `get_invoice_doc(name, "read")` → `frappe.has_permission("CRM Invoice", "read", name)` → `has_invoice_permission` → `has_deal_permission` on `CRM Invoice.deal`. A missing invoice and a forbidden one get the same answer | `CRM Invoice` read; `CRM Company Profile` read (agency's own details, no customer data) | `::TestAC9Permissions::test_the_deals_own_agent_can_read_their_invoice`, `::test_a_sales_user_outside_the_deal_cannot_read_the_invoice`, `::test_a_missing_invoice_is_refused_exactly_like_a_forbidden_one` |
| `crm.api.invoices.get_invoices_for_deal` (GET) | Sales User, Sales Manager, System Manager | `get_deal_doc(deal, "read")` first, then `frappe.get_list` (permission-aware) filtered to that deal. The filter narrows the caller's own scope and cannot widen it | `CRM Deal` read; `CRM Invoice` read through `get_list` | `::TestAC10Flag::test_every_endpoint_refuses_while_the_flag_is_off` |
| `crm.api.invoices.get_next_number` (GET) | Sales User, Sales Manager, System Manager | `frappe.has_permission("CRM Invoice", "create")`. No row scope: the answer is one string derived from the highest serial in the financial year, and it names no record | `CRM Invoice` create; reads `invoice_number` values only, no customer field | `::TestAC2NumberLock::test_the_next_number_endpoint_is_a_preview_and_reserves_nothing` |
| `crm.api.invoices.finalize` (POST) | Sales User, Sales Manager, System Manager | `get_invoice_doc(name, "write")` — the same hook chain, resolved to `write` on the deal | `CRM Invoice` write; `CRM Company Profile` read (Rule 46 supplier fields, checked by `finalize_blockers`) | `::TestAC4B2CThreshold::test_the_endpoint_raises_the_blockers_rather_than_issuing_silently` |
| `crm.api.invoices.record_payment` (POST) | Sales User, Sales Manager, System Manager | `get_invoice_doc(name, "write")`. The schedule row named in the request is checked to BELONG to this invoice before it is stored | `CRM Invoice` write; the optional thank-you goes through `crm.api.email.send_email`, whose `make` checks the `email` permission on `CRM Invoice` and drops suppressed recipients | `::TestAC9Permissions::test_a_sales_user_can_record_a_payment_on_their_own_invoice`, `::test_a_sales_user_outside_the_deal_cannot_record_a_payment`, `::TestAC6Payments::test_the_endpoint_can_send_a_thank_you_and_can_be_asked_not_to` |
| `crm.api.invoices.void_invoice` (POST) | **Sales Manager, System Manager only** | `require_manager()` on `frappe.get_roles()` AND `get_invoice_doc(name, "write")`. Checked TWICE: here and again inside `CRMInvoice.void`, so a future caller that skips the endpoint is still refused | `CRM Invoice` write | `::TestAC9Permissions::test_a_sales_user_cannot_void_even_their_own_invoice`, `::test_the_document_method_refuses_a_sales_user_as_well_as_the_endpoint`, `::test_a_manager_can_void` |
| `crm.api.invoices.set_reminders_paused` (POST) | Sales User, Sales Manager, System Manager | `get_invoice_doc(name, "write")`; one boolean on one named invoice | `CRM Invoice` write | `::TestAC10Flag::test_every_endpoint_refuses_while_the_flag_is_off` |
| `crm.api.invoices.get_tiles` (GET) | Sales User, Sales Manager, System Manager | `frappe.get_list` over `CRM Invoice`, which applies the row conditions above; the payment rows are then read only for the invoice names that survived that filter. **No raw SQL and no query builder** — master spec §3 | `CRM Invoice` read via `get_list`; `CRM Invoice Payment` read scoped by `parent in (permitted names)`; `FCRM Settings` single value (currency) | `::TestAC9Permissions::test_the_tiles_are_scoped_to_the_caller`, `::TestVoidAndTiles::test_void_removes_the_invoice_from_all_three_tiles` |
| `crm.api.invoices.download_invoice` (POST) | Sales User, Sales Manager, System Manager | `get_invoice_doc(name, "read")` AND `frappe.has_permission("CRM Invoice", "print", name)` | `CRM Invoice` read + print; writes one PRIVATE `File` attached to that invoice and nothing else | `::TestAC10Flag::test_every_endpoint_refuses_while_the_flag_is_off` |
| `crm.api.invoices.share_invoice` (POST) | Sales User, Sales Manager, System Manager | `read` AND `print` on the named invoice | `CRM Invoice` read + print; `File` (private) and `CRM Document Link` written with `ignore_permissions=True` for that one record. Minting retires the record's previous live invoice links, so a withdrawn balance cannot still be read at yesterday's URL | `::TestShareLink::test_sharing_mints_a_token_and_keeps_the_file_private`, `::test_re_sharing_retires_the_previous_token` |
| `crm.api.invoices.send_invoice_email` (POST) | Sales User, Sales Manager, System Manager | `get_invoice_doc(name, "write")` AND `print`. **The address comes off the RECORD, never off the request** — the endpoint takes no address argument, asserted by `test_the_email_endpoint_takes_no_address_argument` | `CRM Invoice` write + print; `crm.api.email.send_email` drops suppressed recipients and raises when every recipient is suppressed | `::TestShareLink::test_the_email_send_uses_the_address_on_the_record`, `::test_the_email_endpoint_takes_no_address_argument` |
| `crm.api.invoices.send_invoice_on_whatsapp` (POST) | Sales User, Sales Manager, System Manager | `get_invoice_doc(name, "write")` AND `print`. The number comes from the DEAL through `crm.api.whatsapp.get_reference_whatsapp_numbers`, never from the request; `create_whatsapp_message` enforces that as well | `CRM Invoice` write + print; `CRM Deal` read (for the number); a permission refusal or a send failure retires the link that was minted | `::TestShareLink::test_the_whatsapp_send_uses_a_number_the_deal_already_holds`, `::test_a_whatsapp_failure_retires_the_link_and_reports_the_window` |
| `crm.api.invoices.view` (GET) | **Guest**, deliberately | The token IS the authorization: 128 bits, single-purpose, expiring, revocable, bound to one file. The link's `purpose` is checked, so a quote token cannot be redeemed here. An unknown, a revoked and an expired token all get the same answer, so the route cannot be used to find out which tokens were ever real. No invoice field other than the rendered PDF is reachable through it | `CRM Document Link` resolved by token; `File` streamed; `CRM Document Link View` written | `::TestShareLink::test_a_dead_token_and_an_unknown_token_are_indistinguishable`, `::test_a_live_token_streams_the_file_and_logs_the_view`, `::test_a_quote_token_cannot_be_redeemed_on_the_invoice_route`, `::TestAC10Flag::test_the_public_route_answers_like_a_dead_token_while_the_flag_is_off` |

### Non-endpoint entry points added in Stage 5.3

| Entry point | Reached from | Authorization today | What a future endpoint must add |
| --- | --- | --- | --- |
| `crm.invoice_reminders.send_invoice_reminders` | Frappe scheduler, hourly | Gated on `invoice_reminders_enabled` AND `invoices_enabled`, both default OFF; while either is off it reads no invoice row. Never raises. It reads with `frappe.get_all` (no permission conditions), which is correct only because nothing it reads reaches a request — the only thing it does with an invoice is email the customer whose invoice it already is. It sets `owner_user` to the DEAL'S OWNER, and `crm.outbound.execute_job` re-reads that user AT EXECUTION TIME and refuses a disabled one. Suppression is checked before the job is created and again inside the delivery row lock | A "send this reminder now" endpoint must check `write` on the invoice first, and must not let the caller name the acting user or the address |
| `crm.invoice_reminders.queue_step` / `remind_about` | The sweep above | Same. Writes `CRM Outbound Job` / `CRM Outbound Recipient` with `ignore_permissions=True`, exactly as the sequence adapter does | As above |
| `crm.invoice_reminders.live_link_url` | `message_for`, while a reminder body is built | Strictly READ-ONLY. It reads at most one `CRM Document Link` row for the ONE invoice the sweep is already processing, and returns a URL only when that link is this invoice's, carries the `Invoice` purpose, is `active`, has not expired, and its private File still exists. It never calls `create_link` and never calls `render_print_pdf`, so the sweep cannot mint a token or render a document — asserted by `test_invoices.py::TestAC5ReminderLadders::test_the_sweep_never_mints_a_link_and_never_renders_a_pdf`. The token it returns was already handed to this customer by an agent's own send; the reminder reuses that door rather than opening a new one | An endpoint that returned a token for an invoice a REQUEST named would need `read` + `print` on that invoice first — that endpoint already exists as `crm.api.invoices.share_invoice`, which mints rather than reads |
| `crm.api.invoices.cleanup_invoice_links` | Frappe scheduler, hourly | System job. Retires expired `CRM Document Link` rows and deletes the private `File` each held. Never raises | n/a — scheduler only |
| `crm.api.invoices.install_invoice_print_format` | `after_migrate` | Migration context. Writes one `Print Format` row, and only when the template file changed | n/a — hook only |
| `crm.fcrm.doctype.crm_invoice.crm_invoice.add_invoice_roles` | `after_migrate` | Migration context. Writes `Custom DocPerm` rows. Corrective on every migrate, so a site that ran a looser version is TIGHTENED rather than keeping the old grant | n/a — hook only |
| `crm.fcrm.doctype.crm_sac_code.crm_sac_code.seed_sac_codes` | `after_migrate` | Migration context. Idempotent by code; never overwrites a row an administrator edited | n/a — hook only |
| `crm.fcrm.doctype.crm_invoice.crm_invoice.get_invoice_permission_query_conditions` / `has_invoice_permission` | `permission_query_conditions` / `has_permission` hooks on `CRM Invoice` | These ARE the authorization. Both defer to `crm/permissions/org_hierarchy.py` on `CRM Invoice.deal` | n/a — hooks only |
| `crm.invoicing.*` | The controller, the endpoints, the print format | Pure functions over plain dicts, plus `allocate_number`, which writes only the invoice it was handed. No permission of its own; every caller has already checked | An endpoint that allocated a number directly would have to check `write` on the invoice first |

### Doctype role permissions added in Stage 5.3

| Doctype | System Manager | Sales Manager | Sales User | Why |
| --- | --- | --- | --- | --- |
| `CRM Invoice` | read, create, write, print, email, report, export, share — **no delete** | same — **no delete** | read, create, write, print, email — **no delete, no report, no export, no share** | Design note §Permissions. A Sales User drafts from their own deals and records payments; row scope keeps the list honest, but report and export are the two paths that hand a whole revenue table to a leaving employee. **Nobody** gets delete: `on_trash` refuses for every user including Administrator, and a permission the controller always refuses would be a lie told in the permission screen |
| `CRM Invoice Item` / `Payment` / `Schedule` / `Status Log` | — (child tables) | — | — | Children of the invoice; they inherit the parent's permissions and are saved only through it |
| `CRM SAC Code` | read, create, write, delete, report, export, share | read, create, write, print, email, report, export, share — no delete | **read only** | The codes and rates on it decide what every invoice charges, so it is manager configuration. An agent reads it to pick a code and can change nothing |
| `CRM Company Profile` (Single, added just before this stage) | full | read, create, write, print, email, report, export, share | **read only** | The agency's GSTIN and state code decide the tax split on every invoice. Managers only, as the design note requires |

### One behaviour change to an existing doctype

`CRM Document Link.purpose` gains the option `Invoice`. `crm.api.invoices.view`
checks the purpose before it streams, and `crm.api.quote.view` resolves only its
own links, so a token minted for one kind of document cannot be redeemed on the
other's route (`test_a_quote_token_cannot_be_redeemed_on_the_invoice_route`).
Authorization of the quote route is unchanged.
